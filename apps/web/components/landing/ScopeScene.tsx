...
function ScopeOrbit({ colors }: { colors: BrandColors }) {
  ...
  return <>
    ...
    {/* Contained glowing boundary segment between contract panel and ledger card (no overflow line) */}
    <mesh position={[0.48, 0.1, -0.35]} rotation={[0, 0, 0.08]}><boxGeometry args={[0.04, 0.85, 0.04]} /><meshBasicMaterial color={colors.accent} transparent opacity={0.65} /></mesh>
    <mesh ref={halo} position={[0.48, 0.1, -0.3]} rotation={[0, 0, 0]}><torusGeometry args={[0.55, 0.022, 16, 48]} /><meshBasicMaterial color={colors.accent} transparent opacity={0.35} /></mesh>
    <mesh position={[0.48, 0.1, -0.32]}><sphereGeometry args={[0.12, 16, 16]} /><meshBasicMaterial color={colors.accent} transparent opacity={0.4} /></mesh>
